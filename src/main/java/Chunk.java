...
public abstract class Chunk<K extends Comparable<? super K>,V>
{
	...
	private ThreadData.PutData<K,V>[] putArray;
	...
	public abstract int copyValues(Object[] keys, Object[] values, final int idx, final int myVer, final K min, final K max, final SortedMap<K,ThreadData.PutData<K,V>> items);
	...
	public SortedMap<K, ThreadData.PutData<K,V>> helpPutInScan(int myVersion, K min, K max)
	{
		...
		for (int i = 0; i < MppRunner.NUM_THREADS; ++i)
		{
			...
			ThreadData.PutData<K,V> item = items.get(currKey);
	...
	public ThreadData.PutData<K,V> helpPutInGet(int myVersion, K myKey)
	{
	...
	public void publishPut(ThreadData.PutData<K,V> data)
	{
...
*/
	...
	public V find(K key, ThreadData.PutData<K,V> item)
	{
		// binary search sorted part of order-array to quickly find node to start search at
		// it finds previous-to-key so start with its next
	...
	private V chooseNewer(int item, ThreadData.PutData<K,V> pd)
	{
