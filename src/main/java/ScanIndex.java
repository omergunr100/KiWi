...
public class ScanIndex<K extends Comparable<? super K>> {
    ...
    private Integer max;
    private int currKey;
    public ScanIndex(ArrayList<ThreadData.ScanData> scans, int currVersion, K minKey, K maxKey)
    {
        //this.scans = scans;
        ...
        scans.sort(new Comparator<ThreadData.ScanData>() {
            @Override
            public int compare(ThreadData.ScanData o1, ThreadData.ScanData o2) {
                return o2.version.get() - o1.version.get();
            }
        });
        //for(int i = 0; i < scans.length; ++i)
        for(ThreadData.ScanData sd: scans)
        {
            if(sd == null) continue;
