...
public class ScanIterator<K extends Comparable<? super K>,V> implements Iterator<V>
{
	...
	private final SortedMap<K,ThreadData.PutData<K,V>>		items;	// items map - for items that are currently added (from thread-array)
	...
	public ScanIterator(K min, K max, int version, Chunk<K,V> chunk, SortedMap<K,ThreadData.PutData<K,V>> items)
	{
		this.maxKey = max;
